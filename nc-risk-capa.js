...
function renderRisk() {
  ...
  wrap.innerHTML = rows
    .map(
      ...
      <div class="muted" style="font-size:12px;margin-top:6px;">Owner: ${esc(v(r.riskOwner))}</div>
      <div class="muted" style="font-size:12px;">Mitigation: ${esc(v(r.mitigation))}</div>
      <div style="font-size:12px;margin-top:6px;">${esc(v(r.findings))}</div></div>`
    )
...
["ncrcRiskSearch", "ncrcRiskLevel", "ncrcRiskMitigation"].forEach((id) =>
