...
(function (global) {
  ...
  RULES.push(function riskExposure(ctx) {
    ...
    if (closed >= risks.length * 0.8 && ctx.ncRows.length > risks.length * 0.5) {
      ...
      out.push(finding("Risk", "Medium", "Root Cause",
        single
          ? `Risk is marked closed/mitigated yet non-compliance remains material — verify whether mitigations are truly effective.`
          : `Most risks are marked closed/mitigated yet non-compliance volume remains material — verify whether mitigations are truly effective.`));
    }
    return out;
  });
  ...
  RULES.push(function crossAnalysis(ctx) {
    ...
    if (rcTotal && pct(human, rcTotal) >= 40 && ctx.kpi.compliancePct < TH.compliancePct) {
      const single = rcTotal === 1;
      out.push(finding("Cross-Analysis", "Medium", "Root Cause",
        single
          ? `Human-related root cause accounts for 100% of analyzed items — training & awareness investment likely to lift compliance.`
          : `Human-related root causes account for ${pct(human, rcTotal)}% of analysed items while compliance sits at ${ctx.kpi.compliancePct}% — training & awareness investment likely to lift compliance.`));
    }
  ...
  function buildContext(records) {
    const compBase = records.filter(r => ["Compliant", "Non-Compliant", "Observation"].includes(r.compliance));
