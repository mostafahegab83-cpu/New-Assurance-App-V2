...
(function (global) {
  ...
  RULES.push(function ncCategoryDistribution(ctx) {
    ...
    const prefix = isSingle
      ? `1 NC is with "${t.key}"`
      : `${t.total} NCs are distributed across categories with "${t.key}" leading`;
    const msg = `${prefix} at ${t.share.toFixed(1)}% — focus improvement efforts on this dimension first.`;
