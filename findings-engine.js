...
(function (global) {
  ...
  RULES.push(function ncTypeMix(ctx) {
    ...
    if (share >= 30) {
      return [finding("Non-Compliance", share >= 50 ? "High" : "Medium", "Compliance",
        `Major NCs represent ${major}/${total} (${share}%) of all non-compliances — elevated severity profile.`)];
    }
    return [];
  });
  ...
  RULES.push(function crossAnalysis(ctx) {
    ...
    // Ineffective CAPA + RECURRENT NCs (date-aware)
    // A CAPA is only "weak" if new NCs were raised in audits AFTER the CAPA
    // was closed (or after the audit where it was judged ineffective).
    // Same-cycle clustering (many NCs + 1 ineffective CAPA from the same audit
    // round) is NOT evidence of weakness — there's been no follow-up audit yet.
    ...
    const ineffRefDates = ineffRows
      .map(r => toDate(r.capaCloseDate) || toDate(r.auditDate))
      .filter(Boolean)
      .sort((a, b) => a - b);

    if (ineffRefDates.length > 0) {
      const earliestRef = ineffRefDates[0];
      const recurrentNcs = ctx.ncRows.filter(r => {
        const d = toDate(r.auditDate);
        return d && d > earliestRef;
      }).length;

      if (recurrentNcs >= 3) {
        out.push(finding("Cross-Analysis", "Medium", "Root Cause",
          `${ineffRows.length} CAPA(s) marked ineffective, followed by ${recurrentNcs} new NC(s) ` +
          `raised in later audits (after ${earliestRef.toISOString().slice(0,10)}) — ` +
          `corrective actions are not preventing recurrence.`));
      }
      // else: ineffective CAPA(s) exist but no post-CAPA recurrence yet —
      // insufficient evidence to call corrective actions "weak".
    }
