...
(function (global) {
  ...
  RULES.push(function ncCategoryDistribution(ctx) {
    ...
    if (!t.key || !t.total) return [];
    const msg = t.total === 1
      ? `1 NC  "${t.key}" leading at ${t.share.toFixed(1)}% — focus improvement efforts on this dimension first.`
      : `NCs are distributed across categories with "${t.key}" leading at ${t.share.toFixed(1)}% — focus improvement efforts on this dimension first.`;
    return [finding("Non-Compliance", t.share >= 60 ? "High" : "Low", "Root Cause", msg)];
  });
