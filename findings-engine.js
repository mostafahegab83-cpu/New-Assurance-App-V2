...
(function (global) {
  ...
  RULES.push(function processDelays(ctx) {
    ...
    // "Started on time" = did not start late (on time OR early start). Finishing late then = execution bottleneck.
    const bottlenecks = delayed.filter(p => p.startVariance != null && p.startVariance <= 0);
  ...
  RULES.push(function slaBreaches(ctx) {
    ...
    // Worst process-level end variance for added context
    const delayedProcs = (ctx.processSummary || []).filter(p => p.endVariance != null && p.endVariance > 0);
    const worstProc = delayedProcs.length
      ? delayedProcs.reduce((m, p) => p.endVariance > m.endVariance ? p : m, delayedProcs[0])
      : null;
    const procTail = worstProc
      ? ` At the process level, the largest end-date variance was ${worstProc.endVariance} day(s) on "${worstProc.name}".`
      : "";
    return [finding("SLA", breachPct >= 50 ? "High" : "Medium", "Performance",
      ...
      `Worst step: "${worst.label}" took ${worst.actual} vs target ${worst.target} (${worst.actual - worst.target}-day breach).` +
      procTail)];
  });
  ...
  RULES.push(function riskExposure(ctx) {
    ...
    const closedOnly = risks.filter(r => r.mitigation === "Closed").length;
    const mitigatedOnly = risks.filter(r => r.mitigation === "Mitigated").length;
    const closed = closedOnly + mitigatedOnly;
    if (open > 0) {
    ...
    // Cross signal: time-aware join.
    // An NC is evidence of failed mitigation ONLY if:
    //   - same processName (and same processPhase when both are reported)
    //   - NC.auditDate is strictly AFTER the matching risk's closureDate
    //     (fallback to risk.auditDate when closureDate is blank)
    if (closed >= risks.length * 0.8 && ctx.ncRows.length > risks.length * 0.5) {
      const mitigated = risks.filter(r => r.mitigation === "Closed" || r.mitigation === "Mitigated");
      // Index mitigated risks by processName -> [{ phase, refDate }]
      const riskIdx = new Map();
      for (const r of mitigated) {
        const p = norm(r.processName);
        if (!p) continue;
        const refDate = toDate(r.closureDate) || toDate(r.auditDate);
        if (!refDate) continue;
        if (!riskIdx.has(p)) riskIdx.set(p, []);
        riskIdx.get(p).push({ phase: norm(r.processPhase), refDate });
      }
      // Dedup NCs on (auditDate | processName | processPhase | controlItem)
      const seenNc = new Set();
      const postNcs = [];
      for (const nc of ctx.ncRows) {
        const p = norm(nc.processName);
        const candidates = riskIdx.get(p);
        if (!candidates) continue;
        const ncDate = toDate(nc.auditDate);
        if (!ncDate) continue;
        const ncPhase = norm(nc.processPhase);
        // Pick the earliest refDate among matching phase rows (or any phase if NC phase is blank)
        const matches = candidates.filter(c => !c.phase || !ncPhase || c.phase === ncPhase);
        if (!matches.length) continue;
        const earliestRef = matches.reduce((a, c) => c.refDate < a ? c.refDate : a, matches[0].refDate);
        if (ncDate <= earliestRef) continue; // NC predates mitigation → not evidence
        const k = [dateKey(nc.auditDate), p, ncPhase, norm(nc.controlItem)].join("|");
        if (seenNc.has(k)) continue;
        seenNc.add(k);
        postNcs.push({ proc: p, phase: ncPhase, date: dateKey(nc.auditDate) });
      }
      if (postNcs.length > 0) {
        const sharedProcs = [...new Set(postNcs.map(n => n.proc))];
        out.push(finding("Risk", "Medium", "Root Cause",
          `${closedOnly} risk(s) Closed and ${mitigatedOnly} Mitigated out of ${risks.length} total, ` +
          `yet ${postNcs.length} unique NC(s) were raised AFTER risk closure within ${sharedProcs.length} of the same process(es) ` +
          `(${sharedProcs.join(", ")}) — mitigations are not holding.`));
      }
      // else: no NCs post-date their matching risk closure → stay silent
    }
    return out;
  });
