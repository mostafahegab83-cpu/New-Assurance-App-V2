...
(function (global) {
  ...
  // Escape HTML, then insert a <br> after every N words so long sentences
  // wrap onto multiple lines (prevents text being clipped in PDF export).
  ...
  function render(targetEl, result) {
    ...
    const rows = result.findings.map(f => `
      <tr>
        <td class="fs-cat-cell"><strong>${f.category}</strong><div class="fs-msg">${wrapWords(f.finding, MAX_WORDS)}</div></td>
        <td class="fs-action-cell">${wrapWords(f.action, MAX_WORDS)}</td>
      </tr>`).join("");
