...
(function (global) {
  ...
  const diffDays = (planned, actual) => {
    const p = dateOnlyMs(planned);
    const a = dateOnlyMs(actual);
  ...
  RULES.push(function processDelays(ctx) {
    ...
    // Screenshot logic: any positive end variance means the process finished late.
    const delayed = processSummary.filter(p => p.endVariance != null && p.endVariance > 0);
    ...
    const out = [finding("Process", delayed.length >= 3 ? "High" : "Medium", "Performance",
      `${delayed.length} unique process(es) finished later than planned ` +
      `(avg ${avg}d, max ${max.endVariance}d on "${max.name}").`)];

    // Screenshot logic: start variance 0 or negative = started on time / early.
  ...
  RULES.push(function slaBreaches(ctx) {
    ...
    const procTail = worstProc
      ? ` At the process level, the largest end variance was ${worstProc.endVariance} day(s) on "${worstProc.name}".`
      : "";
    return [finding("SLA", breachPct >= 50 ? "High" : "Medium", "Performance",
      ...
      `Worst step: "${worst.label}" took ${worst.actual} vs target ${worst.target} ` +
      `(${worst.actual - worst.target} day(s) over).${procTail}`)];
  });
