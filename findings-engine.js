...
(function (global) {
  ...
  function slaFinding(records) {
    ...
    const variances = records.map(r => diffDays(r.plannedEndDate, r.actualEndDate)).filter(v => v != null);
    let varianceTxt = "";
    let actionVar = "";
    if (variances.length) {
      const late = variances.filter(v => v > 0);
      if (late.length) {
        const maxV = Math.max.apply(null, late);
        const stepWord = sPl(late.length, "step");
        varianceTxt = ` End Variance shows ${late.length} ${stepWord} finishing late by up to ${maxV} ${sPl(maxV, "day")}.`;
        actionVar = ` Address the ${late.length === 1 ? "delayed step" : "delayed steps"} (worst case ${maxV} ${sPl(maxV, "day")}) by reviewing execution capacity and removing bottlenecks.`;
      } else {
  ...
  function render(targetEl, result) {
    ...
    const rows = result.findings.map(f => `
      <tr>
        <td class="fs-cat-cell"><strong>${f.category}</strong><div class="fs-msg">${f.finding}</div></td>
        <td class="fs-action-cell">${f.action}</td>
      </tr>`).join("");
