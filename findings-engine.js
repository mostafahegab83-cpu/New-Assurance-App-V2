...
(function (global) {
  ...
  function render(targetEl, result) {
    ...
    const rows = result.findings.map(f => `
      <tr>
        <td class="fs-cat-cell" style="width:55%; vertical-align:top; padding:12px 14px; border-bottom:1px solid #e5e7eb; white-space:normal; overflow:visible;">
          <strong style="display:block; margin-bottom:6px;">${escapeHtml(f.category)}</strong>
          <div class="fs-msg" style="${CELL_TEXT_STYLE}">${wrapWords(f.finding)}</div>
        </td>
        <td class="fs-action-cell" style="width:45%; vertical-align:top; padding:12px 14px; border-bottom:1px solid #e5e7eb; white-space:normal; overflow:visible;">
    ...
    targetEl.innerHTML = `
      ...
      <table class="fs-table" style="width:100%; max-width:100%; table-layout:fixed; border-collapse:collapse; overflow:visible;">
        ...
        <thead>
          <tr>
            <th style="width:55%; text-align:left; vertical-align:top; white-space:normal; padding:10px 14px;">Finding</th>
            <th style="width:45%; text-align:left; vertical-align:top; white-space:normal; padding:10px 14px;">Recommended Action</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
