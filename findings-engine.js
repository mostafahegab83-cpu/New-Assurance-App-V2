...
(function (global) {
  ...
  RULES.push(function crossAnalysis(ctx) {
    ...
    /* H5. Process delays ↔ SLA breaches — JOIN on processName.
       Only count SLA-breached steps that belong to a delayed process. */
    const delayedProcs = new Set(
      ctx.processSummary
        .filter(p => p.endVariance != null && p.endVariance > TH.processEndVarianceDays)
        .map(p => norm(p.name))
    );
    if (delayedProcs.size >= 2) {
      const breachesInDelayed = ctx.slaSteps.filter(s => s.actual > s.target && delayedProcs.has(norm(s.processName)));
      if (breachesInDelayed.length >= 2) {
        out.push(finding("Cross-Analysis", "Medium", "Performance",
          `${delayedProcs.size} delayed processes contain ${breachesInDelayed.length} SLA-breached step(s) of their own — ` +
          `process execution issues are cascading into SLA misses within the same processes.`));
      }
      // else: SLA breaches exist elsewhere but not inside the delayed processes → cannot claim cascade.
    }
    return out;
  });
