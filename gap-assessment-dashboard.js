...
function renderHeatmap() {
  ...
  const cell = (v, max) => `<td style="padding:8px;text-align:center;border:1px solid #e5e7eb;background:${heatColor(v,max)};color:${heatText(v,max)};font-weight:700;">${v}</td>`;

  body.innerHTML = rows.map(r => `
    <tr>
      <td style="padding:8px;border:1px solid #e5e7eb;font-weight:600;color:#111827;">${r.owner}</td>
      ${cell(r.Critical, maxes.Critical)}
      ${cell(r.High, maxes.High)}
      ${cell(r.Medium, maxes.Medium)}
      ${cell(r.Low, maxes.Low)}
      <td style="padding:8px;text-align:center;border:1px solid #e5e7eb;font-weight:800;background:#f8fafc;">${r.total}</td>
    </tr>
  `).join("") + `
    <tr style="background:#f1f5f9;font-weight:800;">
      <td style="padding:8px;border:1px solid #e5e7eb;">TOTAL</td>
      <td style="padding:8px;border:1px solid #e5e7eb;text-align:center;">${totals.Critical}</td>
      <td style="padding:8px;border:1px solid #e5e7eb;text-align:center;">${totals.High}</td>
      <td style="padding:8px;border:1px solid #e5e7eb;text-align:center;">${totals.Medium}</td>
      <td style="padding:8px;border:1px solid #e5e7eb;text-align:center;">${totals.Low}</td>
      <td style="padding:8px;border:1px solid #e5e7eb;text-align:center;">${totals.total}</td>
    </tr>
  `;
}
...
function renderInsights() {
  ...
  const evPct = pct(evidence, total);
  const noEvPct = total > 0 ? Math.round(((total - evidence) / total) * 100) : 0;
  ...
  const card = (bg, border, iconBg, icon, title, titleColor, body) => `
    <div style="background:${bg};border:1px solid ${border};border-radius:10px;padding:12px;display:flex;gap:10px;align-items:flex-start;">
      ...
      <div>
        <div style="font-weight:800;color:${titleColor};font-size:13px;margin-bottom:2px;">${title}</div>
  ...
  el.innerHTML = [
    ...
    card("#f0fdf4", "#bbf7d0", "#dcfce7", "📈", "Improve Automation", "#15803d",
      `Automation rate is ${autoPct}%. Opportunity to increase efficiency and control.`),
    card("#eff6ff", "#bfdbfe", "#dbeafe", "💡", "Evidence Gap", "#1d4ed8",
      `${noEvPct}% of processes do not have adequate evidence.`),
  ].join("");
}

/* ---------- Maturity Table ---------- */
function maturityCell(pctVal) {
  let bg, icon, color;
  if (pctVal >= 80)      { bg = "#dcfce7"; icon = "✅"; color = "#15803d"; }
  else if (pctVal >= 60) { bg = "#fef3c7"; icon = "⚠️"; color = "#b45309"; }
  else                    { bg = "#fee2e2"; icon = "❌"; color = "#b91c1c"; }
  return `<td style="padding:12px;text-align:center;background:${bg};color:${color};font-weight:700;border-bottom:1px solid #fff;">
    <span style="display:inline-flex;align-items:center;gap:6px;"><span style="font-size:14px;">${icon}</span> ${pctVal}%</span>
  </td>`;
}
function renderMaturity() {
  const body = document.getElementById("gadMaturityBody");
  if (!body) return;
  const owners = {};
  gaRows.forEach(r => {
    const o = norm(r.processOwner) || "Unassigned";
    if (!owners[o]) owners[o] = { total: 0, pe: 0, ev: 0, au: 0 };
    owners[o].total++;
    if (isYes(r.exists)) owners[o].pe++;
    if (isYes(r.evidenceExists) || isYes(r.hasEvidence) || isYes(r.evidence)) owners[o].ev++;
    if (isYes(r.automated) || lc(r.status).includes("auto")) owners[o].au++;
  });
  const names = Object.keys(owners).sort();
  body.innerHTML = names.map(n => {
    const s = owners[n];
    return `<tr style="background:#fff;">
      <td style="padding:12px;font-weight:700;color:#111827;border-bottom:1px solid #f1f5f9;">${n}</td>
      ${maturityCell(pctInt(s.pe, s.total))}
      ${maturityCell(pctInt(s.ev, s.total))}
      ${maturityCell(pctInt(s.au, s.total))}
    </tr>`;
  }).join("") || `<tr><td colspan="4" style="padding:16px;text-align:center;color:#6b7280;">No data</td></tr>`;
}
