...
function chart4() { // Gauge (semi-doughnut) - Compliance
  const k = computeKpis();
  const pct = k.compliance;
  const color = pct >= 85 ? "#16a34a" : pct >= 70 ? "#eab308" : "#dc2626";
  destroy("gadChart4");
  charts.gadChart4 = new Chart(document.getElementById("gadChart4"), {
    type: "doughnut",
    data: {
      labels: ["Compliance", "Gap"],
      datasets: [{ data: [pct, 100 - pct], backgroundColor: [color, "#e5e7eb"], borderWidth:0, circumference:180, rotation:270 }],
    },
    options: {
    ...
    plugins: [{
      ...
      afterDraw(chart) {
        ...
        ctx.fillText(pct + "%", width/2, height*0.85);
        ctx.fillStyle = "#6b7280";
        ctx.font = "13px sans-serif";
        ctx.fillText("Compliance", width/2, height*0.95);
        ctx.restore();
      }
    }],
  });
...
function renderAll() {
  ...
  requestAnimationFrame(() => { chart1(); chart2(); chart3(); chart4(); chart5(); });
}
