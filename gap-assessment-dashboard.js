...
function chart3() { // Bar — Missing processes by Process Owner
  const byOwner = {};
  gaRows.forEach(r => {
  ...
  charts.gadChart3 = new Chart(document.getElementById("gadChart3"), {
    type: "bar",
    data: { labels: owners, datasets: [{ label: "Missing Processes", data, backgroundColor: colors, borderWidth: 1 }] },
    options: {
      responsive: true,
...
function renderAll() {
  ...
  const kpiTotal = document.getElementById("gadKpiTotal");
  const kpiCrit  = document.getElementById("gadKpiCritical");
  const critMissing = gaRows.filter(r => isNo(r.exists) && severityOf(r) === "Critical").length;
  if (kpiTotal) kpiTotal.textContent = gaRows.length;
  if (kpiCrit)  kpiCrit.textContent  = critMissing;
