...
(function () {
  ...
  function renderRecords() {
    ...
    if (!records.length) {
      tbody.innerHTML = `<tr><td colspan="32" style="text-align:center;color:var(--muted);padding:24px">No records yet. Use the “Add / Edit” tab to create one.</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = records.map(r => {
      ...
      <tr>
        ...
        <td>${escHtml(r.ncCategory)}</td>
        <td>${escHtml(r.ncResponsibleDept)}</td>
        <td class="wrap">${escHtml(r.ncDesc)}</td>
        <td>${badge("severityLevel", r.severityLevel)}</td>
        <td>${escHtml(r.verifiedBy)}</td>
        ...
        <td>${escHtml(r.capaIdField)}</td>
        <td>${escHtml(r.rcCategory)}</td>
        <td>${badge("capaStatus", r.capaStatus)}</td>
