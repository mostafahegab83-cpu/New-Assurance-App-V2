...
(function () {
  ...
  const VIEWER_TABS = ["dashboard", "gapAnalysis"];
  const VIEWER_GA_SUBS = ["gapAssessmentDashboard", "gapDashboard"];

  function applyViewerRestrictions(on) {
    document.body.classList.toggle("viewer-only", !!on);

    document.querySelectorAll('[data-tab]').forEach(btn => {
      const allowed = !on || VIEWER_TABS.includes(btn.getAttribute("data-tab"));
      if (btn.hasAttribute("data-admin-only")) return; // handled by admin toggle
      btn.style.display = allowed ? "" : "none";
    });

    document.querySelectorAll('[data-gasub]').forEach(btn => {
      const allowed = !on || VIEWER_GA_SUBS.includes(btn.getAttribute("data-gasub"));
      btn.style.display = allowed ? "" : "none";
    });

    if (!on) return;

    // Land on the Dashboard tab and default the Gap Analysis sub-tab to a dashboard
    document.querySelector('[data-gasub="gapAssessmentDashboard"]')?.click();
    document.querySelector('[data-dashsub="dashFindings"]')?.click();
    document.querySelector('[data-tab="dashboard"]')?.click();
  }

  // Block any attempt to reach a hidden tab / sub-tab
  document.addEventListener("click", (ev) => {
    if (!window.__session?.isViewerOnly) return;
    const btn = ev.target.closest("[data-tab],[data-gasub]");
    if (!btn) return;
    const tab = btn.getAttribute("data-tab");
    const gasub = btn.getAttribute("data-gasub");
    const blocked = (tab && !VIEWER_TABS.includes(tab)) || (gasub && !VIEWER_GA_SUBS.includes(gasub));
    if (blocked) {
      ev.preventDefault();
  ...
  observeAuthState(async (user) => {
    if (user) {
      ...
      const viewerOnly = !admin && VIEWER_EMAILS.map(e => String(e).toLowerCase()).includes(email);

      window.__session = { user, email, isAdmin: admin, isViewerOnly: viewerOnly };
      ...
      applyViewerRestrictions(viewerOnly);
      overlay.style.display = "none";
      window.__initAdminPanel?.();
      window.__startComplianceSync?.();
    } else {
      window.__session = { user: null, email: null, isAdmin: false, isViewerOnly: false };
