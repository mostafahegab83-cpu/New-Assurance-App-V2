...
(function () {
  ...
  /* ---------- Filters & Dashboard ---------- */
  const FILTER_IDS = ["filterDepartment","filterProcess","filterProcessOwner","filterCompliance","filterRiskLevel","filterMitigation"];
  ...
  function refreshFindings(data) {
    ...
    try {
      ...
      if (target) window.FindingsEngine.render(target, _lastFindings, { categoryExclude: "Risk" });
      const riskTarget = document.getElementById("findingsSectionRisk");
      if (riskTarget) window.FindingsEngine.render(riskTarget, _lastFindings, { categoryFilter: "Risk" });
  ...
  async function exportPdf() {
    ...
    // Scope toggling (kept for non-"all" scopes)
    ...
    try {
      ...
      const chartIds = [
        ...
        "chartCompliance","chartNcCategory",
      ...
      const complianceRow = getSharedChartRow(["chartCompliance","chartNcCategory","chartTaskPerformance"]);
      ...
      let orderedBlocks;
      if (scope === "all") {
        orderedBlocks = [header, kpis, scheduleVar, complianceRow, capaRow, riskRow, findings];
      } else {
        ...
        if (scope === "charts") orderedBlocks.push(complianceRow, capaRow, riskRow);
  ...
  function exportCSV() {
    const anyFilter = !!document.getElementById("filterSearch").value
      || ["filterDepartment","filterProcess","filterProcessOwner","filterCompliance","filterRiskLevel","filterMitigation"].some(id => msValues(id).length);
