  ...
  async function exportPdf() {
    ...
    try {
      ...
      async function renderBlock(el) {
        ...
        const scale = Math.max(1.5, Math.min(2.5, maxCanvasSide / longestSide));
      ...
      for (let i = 0; i < rendered.length; i++) {
        ...
        const img = item.canvas.toDataURL("image/jpeg", 0.94);
        pdf.addImage(img, "JPEG", x, cursorY, drawW, drawH);
        cursorY += drawH + gap;
        pageHasContent = true;
      }
