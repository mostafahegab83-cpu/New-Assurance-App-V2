...
(function () {
  ...
  function renderScheduleVariance(data) {
    ...
    data.forEach(r => {
      ...
      if (!groups[name]) groups[name] = { startDeltas: [], endDeltas: [], durDeltas: [], total: 0 };
      groups[name].total++;
      const sd = diffDays(r.plannedStartDate, r.actualStartDate);
      const ed = diffDays(r.plannedEndDate, r.actualEndDate);
      const plannedDur = diffDays(r.plannedStartDate, r.plannedEndDate);
      const actualDur  = diffDays(r.actualStartDate,  r.actualEndDate);
      if (sd !== null) groups[name].startDeltas.push(sd);
      if (ed !== null) groups[name].endDeltas.push(ed);
      if (plannedDur !== null && actualDur !== null) groups[name].durDeltas.push(actualDur - plannedDur);
    });
    ...
    tbody.innerHTML = names.map(n => {
      ...
      return `<tr>
        <td>${escHtml(n)}</td>
        <td>${g.total}</td>
        <td>${avg(g.startDeltas)}</td>
        <td>${avg(g.endDeltas)}</td>
        <td>${avg(g.durDeltas)}</td>
      </tr>`;
    }).join("");
  }
