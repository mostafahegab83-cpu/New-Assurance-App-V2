...
(function () {
  ...
  function renderDashboard() {
    ...
    // ---------- Charts ----------
    drawDoughnut("chartCompliance", counts(data, "compliance", false));
    drawSlaByPhase("chartSlaPerf", data);

    drawBar("chartNcSeverity", counts(ncRows, "severityLevel", false));
    drawStackedByDept("chartNcDept", ncRows);
    drawDoughnut("chartNcCategory", counts(ncRows, "ncCategory", false));

    drawPareto("chartPareto", counts(ncRows, "rootCause", false));

    const riskData = data.filter(r => r.riskExist === "Yes");
    drawHeatmap("riskHeatmap", riskData);
    drawDoughnut("chartRiskType", counts(riskData, "riskType", false));
    drawBar("chartRiskOwner", counts(riskData, "riskOwner", false));

    drawDoughnut("chartCapaStatus", counts(capaNeededRows, "capaStatus", false));
    drawCapaAging("chartCapaAging", capaNeededRows);

    drawPhaseStacked("chartPhasePerf", data);
    drawDeptScorecard("chartDeptScore", data);
    drawTrends("chartTrends", data);

    // ---------- Executive Table ----------
    const exec = data.filter(r =>
      ["High","Critical","Major"].includes(r.severityLevel) || r.riskLevel === "High"
    );
    const tbody = document.querySelector("#pdfTable tbody");
    tbody.innerHTML = exec.map(r => `
      <tr>
        <td>${escHtml(r.auditId)}</td>
        <td>${escHtml(r.auditDate)}</td>
        <td>${escHtml(r.processName)}</td>
        <td>${escHtml(r.department)}</td>
        <td>${escHtml(r.ncId)}</td>
        <td>${escHtml(r.severityLevel)}</td>
        <td>${escHtml(r.riskLevel)}</td>
        <td>${escHtml(r.capaStatus)}</td>
      </tr>`).join("");
  }
  ...
  function exportCSV() {
    const anyFilter = ["filterSearch","filterDepartment","filterProcess","filterCompliance","filterRiskLevel","filterMitigation"]
