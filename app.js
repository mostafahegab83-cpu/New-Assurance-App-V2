...
(function () {
  ...
  function renderDashboard() {
    ...
    colorCodeKpi("kpiEffectiveCapa", effPct);
  ...
  async function exportPdf() {
    ...
    let riskPdfHeader = null;
    if (meta) {
      meta.textContent = `Exported: ${new Date().toLocaleString()} · Filters: ${activeFiltersText()}`;
    }
    if (riskActive) {
      riskPdfHeader = document.createElement("div");
      riskPdfHeader.id = "riskPdfHeader";
      riskPdfHeader.style.cssText = "display:flex;align-items:center;gap:14px;padding:14px 16px;margin-bottom:14px;background:#fff;border:1px solid #e3e8ef;border-radius:8px;";
      riskPdfHeader.innerHTML = `
        <img src="idh-logo.png" alt="IDH" style="height:44px;width:auto;display:block;" />
        <div>
          <h2 style="margin:0;font-size:16px;color:#1f2937;">Assurance Tracker — Dashboard</h2>
          <p style="margin:2px 0 0;font-size:12px;color:#6b7280;">${meta ? meta.textContent : `Exported: ${new Date().toLocaleString()} · Filters: ${activeFiltersText()}`}</p>
        </div>`;
      capture.insertBefore(riskPdfHeader, capture.firstChild);
    }
    ...
    try {
      ...
      if (riskActive) {
        ...
        orderedBlocks = [riskPdfHeader, riskKpis, riskRow, riskFindings];
      } else {
      ...
      if (riskPdfHeader) riskPdfHeader.remove();
