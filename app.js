...
(function () {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      // Capture each dashboard section as ONE block so the on-screen layout
      // (3 charts per row, tight spacing) is preserved exactly in the PDF.
      // No pushChildren — that broke sections into stacked single cards and
      // produced the blank-space + duplication issues.
      const blocks = [];
      ...
      if (scope === "all" || scope === "charts")
        pushBlock(capture.querySelector('[data-section="charts"]'));
