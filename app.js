...
(function () {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      // ---- Collect blocks in DOM order ----
      const blocks = [];
      const header = document.getElementById("pdfHeader");
      if (header) blocks.push(header);
      capture.querySelectorAll("[data-section]").forEach(sec => {
        if (sec.style.display === "none") return;
        if (sec.dataset.section === "charts") {
          Array.from(sec.children).forEach(child => blocks.push(child));
        } else {
          blocks.push(sec);
        }
      });
      ...
      // Page 1: everything up to and including the FIRST `.charts` row (Compliance & NC).
      // Page 2+: remaining blocks (CAPA heading + row, Risk heading + row, ...).
      const pages = [[]];
      let chartsRowsSeen = 0;
      let pageBreakArmed = false;
      for (const el of blocks) {
        if (pageBreakArmed) {
          pages.push([]);
          pageBreakArmed = false;
        }
        pages[pages.length - 1].push(el);
        if (el.classList && el.classList.contains("charts")) {
          chartsRowsSeen += 1;
          if (chartsRowsSeen === 1) pageBreakArmed = true;
        }
      }

      // ---- Render each page: capture all blocks, then scale group to fit ----
      for (let p = 0; p < pages.length; p++) {
        if (p > 0) pdf.addPage();
        const pageBlocks = pages[p].filter(el => el && (el.id === "pdfHeader" || el.offsetParent !== null));
        if (!pageBlocks.length) continue;

        const rendered = [];
        for (const el of pageBlocks) {
          const canvas = await html2canvas(el, { scale: 2, backgroundColor: "#ffffff", useCORS: true, logging: false });
          rendered.push({ canvas, ratio: canvas.height / canvas.width });
        }

        // Natural heights if each image used full contentW
        const naturalHeights = rendered.map(r => contentW * r.ratio);
        const totalGap = gap * (rendered.length - 1);
        const totalNatural = naturalHeights.reduce((a, b) => a + b, 0) + totalGap;

        // Scale down uniformly if the group is taller than one page
        const scale = totalNatural > contentH ? contentH / totalNatural : 1;
        const widthScaled = contentW * scale;

        let cursorY = margin;
        for (let i = 0; i < rendered.length; i++) {
          const r = rendered[i];
          const imgH = naturalHeights[i] * scale;
          const x = margin + (contentW - widthScaled) / 2;
          const img = r.canvas.toDataURL("image/jpeg", 0.92);
          pdf.addImage(img, "JPEG", x, cursorY, widthScaled, imgH);
          cursorY += imgH + gap;
        }
      }
