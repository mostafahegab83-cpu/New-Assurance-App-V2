...
(function () {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      const pdf = new jsPDF({ orientation, unit: "pt", format: "a4", compress: false });
      ...
      if (scope === "all" || scope === "kpis") {
        addBlock(capture.querySelector('[data-section="kpis"]'));
      }
      ...
      if (scope === "all" || scope === "charts") {
        ...
        // Ensure the Task Performance chart is included even if its wrapper
        // lives outside the charts section in the DOM.
        const tpCanvas = document.getElementById("chartTaskPerformance");
        if (tpCanvas) {
          const wrapper = tpCanvas.closest('.chart-card, .card, .dash-card, [data-chart-card], section, .panel') || tpCanvas.parentElement;
          if (wrapper && isVisible(wrapper) && !added.has(wrapper)) {
            // Avoid double-adding if an ancestor was already pushed.
            let alreadyCovered = false;
            added.forEach(a => { if (a.contains(wrapper)) alreadyCovered = true; });
            if (!alreadyCovered) addBlock(wrapper);
          }
        }
      }
      ...
      async function renderBlock(el) {
        ...
        const longestSide = Math.max(rect.width || el.scrollWidth || 1, rect.height || el.scrollHeight || 1);
        const scale = Math.max(3, Math.min(4, maxCanvasSide / longestSide));
        ...
        const naturalH = contentW * (canvas.height / canvas.width);
        return { canvas, naturalH };
      }

      const rendered = [];
      for (const block of blocks) {
        rendered.push({ ...block, ...(await renderBlock(block.el)) });
      }
      ...
      // Helper: slice a tall canvas across multiple pages so we don't leave
      // blank space at the bottom of a page just because the block is big.
      function placeItem(item) {
        const naturalH = item.naturalH;
        // Small enough to fit on one page at full width — use the original
        // single-shot path so we keep crisp rendering.
        if (naturalH <= contentH) {
          let drawW = contentW;
          let drawH = naturalH;
          const remainingH = margin + contentH - cursorY;
          if (pageHasContent && drawH > remainingH) addNewPage();
          const x = margin + (contentW - drawW) / 2;
          const img = item.canvas.toDataURL("image/png");
          pdf.addImage(img, "PNG", x, cursorY, drawW, drawH, undefined, "NONE");
          cursorY += drawH + gap;
          pageHasContent = true;
          return;
        }

        // Block is taller than a page — slice across pages.
        const canvas = item.canvas;
        const pxPerPt = canvas.width / contentW;            // image pixels per PDF point
        let srcY = 0;
        const totalSrcH = canvas.height;
        while (srcY < totalSrcH) {
          const remainingPt = margin + contentH - cursorY;
          // If there's almost no room left, move to a new page.
          if (remainingPt < 80) { addNewPage(); continue; }
          const sliceHpx = Math.min(totalSrcH - srcY, Math.floor(remainingPt * pxPerPt));
          ...
          const drawH = sliceHpx / pxPerPt;
          const img = sliceCanvas.toDataURL("image/png");
          pdf.addImage(img, "PNG", margin, cursorY, contentW, drawH, undefined, "NONE");
          cursorY += drawH + gap;
          pageHasContent = true;
          srcY += sliceHpx;
          if (srcY < totalSrcH) addNewPage();
        }
      }

      for (let i = 0; i < rendered.length; i++) {
        const item = rendered[i];
        if (item.startOnNewPage && pageHasContent) addNewPage();

        // Keep-with-next: pull a small header onto the next block's page
        // when it would otherwise be stranded at the bottom.
        const next = item.keepWithNext ? rendered[i + 1] : null;
        if (next && pageHasContent && item.naturalH <= contentH) {
          const afterY = cursorY + item.naturalH + gap;
          const nextFits = (margin + contentH - afterY) >= Math.min(next.naturalH, 80);
          if (!nextFits) addNewPage();
        }
