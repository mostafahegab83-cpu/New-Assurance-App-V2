...
(function () {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      // Build ordered list of "blocks". Each block = { el, breakAfter }.
      // After the first row of charts ("Compliance & NC"), force a page break
      // so the remaining chart groups (CAPA, Risk) start on page 2.
      const blocks = [];
      const header = document.getElementById("pdfHeader");
      if (header) blocks.push({ el: header });
      capture.querySelectorAll("[data-section]").forEach(sec => {
        ...
        if (sec.dataset.section === "charts") {
          ...
          Array.from(sec.children).forEach(child => {
            const block = { el: child };
            blocks.push(block);
            if (child.classList && child.classList.contains("charts")) {
              chartsRowSeen += 1;
              // After the first charts row (Compliance & NC), break the page.
              if (chartsRowSeen === 1) block.breakAfter = true;
            }
          });
        } else {
          blocks.push({ el: sec });
      ...
      for (const block of blocks) {
        const el = block.el;
        if (!el || (el.offsetParent === null && el.id !== "pdfHeader")) continue;
        ...
        pdf.addImage(img, "JPEG", x, cursorY, imgW, imgH);
        cursorY += imgH + gap;
        firstOnPage = false;
