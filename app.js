...
(function () {
  ...
  function renderScheduleVariance(data) {
    ...
    tbody.innerHTML = names.map(n => {
      ...
      return `<tr>
        <td>${escHtml(n)}</td>
        <td style="text-align:center;">${sv === null ? "—" : sv}</td>
        <td style="text-align:center;">${ev === null ? "—" : ev}</td>
      </tr>`;
    }).join("");
  }
