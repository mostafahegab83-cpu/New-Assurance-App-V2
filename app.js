  ...
  async function exportPdf() {
    ...
    try {
      ...
      for (let i = 0; i < rendered.length; i++) {
        ...
        let drawW = contentW;
        let drawH = item.naturalH;

        // Force a new page when requested (e.g. Findings Summary section).
        if (item.startOnNewPage && pageHasContent) {
          addNewPage();
        }

        // Scale-down oversized blocks so they fit on a single page.
        if (drawH > contentH) {
          const factor = contentH / drawH;
          drawW = contentW * factor;
          drawH = contentH;
        }

        // Only break to a new page if the CURRENT block doesn't fit in
        // what's left of this page. Do NOT consider the next block's size
        // here — that was leaving most of page 1 blank.
        const remainingH = margin + contentH - cursorY;
        if (pageHasContent && drawH > remainingH) {
          addNewPage();
        }

        // Keep-with-next: if the next sibling is a small header that would
        // otherwise be the last thing on the page, pull it down with us.
        const next = item.keepWithNext ? rendered[i + 1] : null;
        if (next && pageHasContent) {
          const afterY = cursorY + drawH + gap;
