...
(function () {
  ...
  function showChartDrilldown(title, parentLabel, breakdown, color) {
    ...
    if (!modal) {
      ...
      modal.innerHTML = '<div id="chartDrilldownBox" style="background:#fff;border-radius:10px;max-width:560px;width:92%;max-height:80vh;overflow:auto;box-shadow:0 10px 30px rgba(0,0,0,.25);"></div>';
    ...
    const rows = breakdown.length
      ? breakdown.map(b => {
          ...
          return `<div style="margin:8px 0;">
            <div style="display:flex;justify-content:space-between;font-size:13px;color:#111;">
              <span style="flex:1;padding-right:8px;">${escH(b.label)}</span>
      ...
      : '<div style="color:#777;font-size:13px;">No sub-category data recorded for this slice.</div>';
    box.innerHTML = `
      <div style="padding:14px 18px;border-bottom:1px solid #eee;display:flex;align-items:center;gap:10px;">
        ...
        <div style="flex:1;">
          <div style="font-weight:700;color:#111;">${escH(title)}</div>
          <div style="font-size:12px;color:#666;">${escH(parentLabel)} — Total: ${total}</div>
        </div>
        <button type="button" id="chartDrilldownClose" style="border:0;background:#f3f4f6;border-radius:6px;padding:4px 10px;cursor:pointer;">Close</button>
      </div>
    ...
    charts[id] = new Chart(ctx, {
      ...
      options: {
        ...
        onClick: (evt, elements, chart) => {
          ...
          const subRows = drill.rows.filter(r => String(r[drill.parentField] || "").trim().toLowerCase() === labelKey);
          const counts = {};
          subRows.forEach(r => {
            const k = String(r[drill.subField] || "").trim() || "N/A";
            counts[k] = (counts[k] || 0) + 1;
          });
          const breakdown = Object.keys(counts)
            .map(k => ({ label: k, count: counts[k] }))
            .sort((a, b) => b.count - a.count);
          showChartDrilldown(drill.title || "Breakdown", label, breakdown, color);
        },
