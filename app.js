...
(function () {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      const blocks = [];
      const pushBlock = el => { if (isVisible(el) && !blocks.includes(el)) blocks.push(el); };
      const pushChildren = container => {
        if (!isVisible(container)) return;
        let kids = Array.from(container.children).filter(isVisible);
        if (kids.length === 1) {
          const nested = Array.from(kids[0].children || []).filter(isVisible);
          if (nested.length > 1) kids = nested;
        }
        if (!kids.length) { pushBlock(container); return; }
        kids.forEach(pushBlock);
      };
      ...
      if (scope === "all" || scope === "taskPerformance" || scope === "charts")
        pushBlock(capture.querySelector('[data-section="taskPerformance"]'));
      if (scope === "all" || scope === "charts")
        pushChildren(capture.querySelector('[data-section="charts"]'));
      if (scope === "all" || scope === "findings")
        pushChildren(document.getElementById("findingsSection"));

      // Force-include the Task Performance chart node itself. Some layouts keep
      // the section wrapper visible while the actual chart sits inside an auto-
      // created card/canvas, so capturing only the wrapper can miss the donut.
      const taskChart = document.getElementById("chartTaskPerformance");
      const taskChartBlock = taskChart?.closest('[data-auto-chart="taskPerformance"], .chart-card, .card, .dash-card, .panel')
        || taskChart?.closest('[data-section="taskPerformance"]')
        || taskChart;
      if (scope === "all" || scope === "taskPerformance" || scope === "charts") {
        pushBlock(taskChartBlock);
      }

      if (!blocks.length) throw new Error("Nothing visible to export");

      const groupedFindings = document.getElementById("findingsSection");
      if (isVisible(groupedFindings) && (scope === "all" || scope === "findings")) {
        const findingCards = Array.from(groupedFindings.querySelectorAll(".finding-card, .card, table, .insight-card")).filter(isVisible);
        if (findingCards.length >= 2) {
          const alreadyAdded = new Set(blocks);
          findingCards.forEach((el) => {
            if (!alreadyAdded.has(el)) blocks.push(el);
          });
        }
      }
