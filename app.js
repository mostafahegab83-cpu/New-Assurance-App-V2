...
(function () {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      // Build ordered list of "blocks" so charts/tables are never split mid-element.
      const blocks = [];
      ...
      capture.querySelectorAll("[data-section]").forEach(sec => {
        ...
        if (sec.dataset.section === "charts") {
          // Walk children: titles + each row of chart-cards, and split rows into individual cards
          Array.from(sec.children).forEach(child => {
            if (child.classList.contains("charts")) {
              Array.from(child.children).forEach(card => blocks.push(card));
            } else {
              blocks.push(child);
            }
          });
      ...
      for (const el of blocks) {
        ...
        // If a single block is taller than a page, scale it down to fit one page
        if (imgH > contentH) {
