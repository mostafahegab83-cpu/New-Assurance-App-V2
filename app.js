...
(function () {
  ...
  const FIELDS = [
    ...
    { id: "processOwner",   label: "Process Owner",        kind: "text" },
  ...
  function renderRecords() {
    ...
    if (!renderRecords._filtersWired) {
      const ids = ["recFilterAuditId","recFilterUser","recFilterProcess","recFilterPhase"];
      ids.forEach(id => {
    ...
    const fProc    = fv("recFilterProcess");
  ...
  function renderDashboard() {
    ...
    const CAT_NC_CATEGORY = { labels:["People","Process","System","Others"],
                              colors:["#2563eb","#16a34a","#f97316","#6b7280"] };
    ...
    const CAT_NC_REASON   = { labels:["Missing Evidence","SLA Breach","Incomplete forms","Outdated SOPs",
                                      "Unapproved documents used by employees","Missing signatures or approvals",
                                      "Version control problems","Missing ticket resolution","Others"],
                              colors:["#dc2626","#f59e0b","#2563eb","#16a34a","#06b6d4","#7c3aed","#db2777","#d97706","#6b7280"] };
    const CAT_RC_CATEGORY = { labels:["Lack of training","Human error","Process","System/Technology","Others"],
                              colors:["#dc2626","#f59e0b","#16a34a","#2563eb","#6b7280"] };
    ...
    drawPieCountPct("chartNcReason", counts(ncReasonRows, "complianceReason", false), CAT_NC_REASON);
  ...
  function refreshFindings(data) {
    ...
    try {
      ...
      if (target) window.FindingsEngine.render(target, _lastFindings);
  ...
  function drawPieCountPct(id, obj, spec) {
  ...
  function activeFiltersText() {
    const parts = [];
    const map = { filterSearch: "Search", filterDepartment: "Action Owner",
  ...
  async function exportPdf() {
    ...
    if (meta) {
      meta.textContent = `Exported: ${new Date().toLocaleString()} · Filters: ${activeFiltersText()}`;
    }

    // Make sure the Findings Summary is current before we snapshot the page.
    refreshFindings(getFiltered());

    // Make sure the Task Performance donut is up-to-date on screen so it
    // is captured by html2canvas (this is the chart that "disappeared").
    try {
      drawTaskPerformanceDonut("chartTaskPerformance", getFiltered());
      charts.chartTaskPerformance?.update?.("none");
    } catch (_) {}

    // Toggle section visibility based on scope (task performance always travels
    // with the "charts" scope so the donut is never skipped).
    ...
    try {
      ...
      // Give the browser a tick to apply visibility / let charts settle.
      ...
      const chartBlocks = [];
      const pushBlock = el => {
        if (!isVisible(el) || blocks.includes(el)) return;
        blocks.push(el);
      };
      const pushChartBlock = (el, options = {}) => {
        if (!isVisible(el)) return;
        const existing = chartBlocks.find(item => item.el === el);
        if (existing) {
          existing.fullRow = existing.fullRow || !!options.fullRow;
          return;
        }
        chartBlocks.push({ el, fullRow: !!options.fullRow });
      };
      const chartIds = [
      ...
      const getSharedChartRow = (ids) => {
        ...
        while (ancestor && ancestor !== capture) {
          if (cards.every(card => ancestor.contains(card))) {
            const chartCount = chartIds.filter((chartId) => {
              const chartNode = document.getElementById(chartId);
              return chartNode && ancestor.contains(chartNode);
            }).length;
      ...
      pushBlock(document.getElementById("pdfHeader"));
      if (scope === "all" || scope === "kpis") {
        pushBlock(capture.querySelector('[data-section="kpis"]'));
      }
      if (scope === "all" || scope === "table") {
        pushBlock(capture.querySelector('[data-section="scheduleVariance"]'));
      }
      if (scope === "all" || scope === "charts") {
        const riskChartIds = ["chartRiskType", "chartRiskLevel", "chartRiskMitigation"];
        const riskRow = getSharedChartRow(riskChartIds);
        const groupedChartIds = new Set(riskRow ? riskChartIds : []);

        chartIds.forEach((id) => {
          if (riskRow && id === riskChartIds[0]) {
            pushChartBlock(riskRow, { fullRow: true });
            return;
          }
          if (groupedChartIds.has(id)) return;
          pushChartBlock(getChartCard(id));
        });
      } else if (scope === "taskPerformance") {
        pushChartBlock(getChartCard("chartTaskPerformance"));
      }
      if (scope === "all" || scope === "findings") {
        pushBlock(document.getElementById("findingsSection"));
      }

      if (!blocks.length && !chartBlocks.length) throw new Error("Nothing visible to export");

      // Give the donut one more paint frame before capture so the canvas never
      // snapshots mid-render as blank.
      await new Promise(requestAnimationFrame);

      // Render each block to a canvas at a fixed scale.
      const rendered = [];
      for (const el of blocks) {
        ...
        const heightPt = contentW * (canvas.height / canvas.width);
        rendered.push({ canvas, heightPt });
      }
      const renderedCharts = [];
      for (const item of chartBlocks) {
      ...
      // Pack blocks onto pages with tight gaps. Blocks taller than a page are
      // sliced across pages so each page fills edge-to-edge — eliminates the
      // big blank space at the bottom of every page.
      const gap = 4;
      let cursorY = margin;
      let pageHasContent = false;
      const newPage = () => { pdf.addPage(); cursorY = margin; pageHasContent = false; };

      for (const item of rendered) {
        const { canvas, heightPt } = item;
        const pxPerPt = canvas.width / contentW;
        let remaining = heightPt;
        let srcYpx = 0;

        while (remaining > 0) {
          const available = (margin + contentH) - cursorY;
          const placeH = Math.min(remaining, available);
          // If almost no room left, jump to next page rather than squeezing a sliver.
          if (placeH < 40 && pageHasContent) { newPage(); continue; }

          const slicePx = Math.round(placeH * pxPerPt);
          let drawData;
          if (slicePx >= canvas.height && srcYpx === 0) {
            ...
            drawData = slice.toDataURL("image/png");
          }
          pdf.addImage(drawData, "PNG", margin, cursorY, contentW, placeH, undefined, "FAST");
          cursorY += placeH + gap;
          srcYpx += slicePx;
          remaining -= placeH;
          pageHasContent = true;
      ...
      if (renderedCharts.length) {
        ...
        const colGap = 10;
        const rowGap = 12;
        const cellW = (contentW - colGap * (cols - 1)) / cols;
        let row = [];

        const flushGridRow = () => {
          if (!row.length) return;
          const rowHeights = row.map((item) => cellW * (item.canvas.height / item.canvas.width));
          const rowH = Math.max(...rowHeights);
          const available = (margin + contentH) - cursorY;

          if (pageHasContent && available < rowH + rowGap) {
            newPage();
          }

          row.forEach((item, index) => {
            const x = margin + index * (cellW + colGap);
            const h = cellW * (item.canvas.height / item.canvas.width);
            pdf.addImage(item.dataUrl, "PNG", x, cursorY, cellW, h, undefined, "FAST");
          });
        ...
        for (const item of renderedCharts) {
          if (item.fullRow) {
            flushGridRow();
            const drawH = contentW * (item.canvas.height / item.canvas.width);
            const available = (margin + contentH) - cursorY;

            if (pageHasContent && available < drawH + rowGap) {
              newPage();
            }

            pdf.addImage(item.dataUrl, "PNG", margin, cursorY, contentW, drawH, undefined, "FAST");
            cursorY += drawH + rowGap;
            pageHasContent = true;
            continue;
          }

          row.push(item);
          if (row.length === cols) flushGridRow();
        }
