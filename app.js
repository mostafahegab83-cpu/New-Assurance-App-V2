...
(function () {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      const isVisibleEl = isVisible;
      const chartIds = [
        "chartTaskPerformance",
        "chartCompliance","chartNcCategory","chartSlaPerf",
        "chartNcReason","chartRcCategory","chartCapaStatus",
      ...
      // Page 1: header + KPIs + Process Timeline + Task Performance
      // Page 2: Compliance & NC row + CAPA row
      // Page 3: Risk row + Findings Summary
      ...
      const complianceRow = getSharedChartRow(["chartCompliance","chartNcCategory","chartSlaPerf"]);
      const capaRow = getSharedChartRow(["chartNcReason","chartRcCategory","chartCapaStatus"]);
      ...
      const pageGroups = [];
      if (scope === "all") {
        pageGroups.push([header, kpis, scheduleVar, taskCard].filter(isVisibleEl));
        pageGroups.push([complianceRow, capaRow].filter(isVisibleEl));
        pageGroups.push([riskRow, findings].filter(isVisibleEl));
      } else {
        // Fallback: single-page best-effort with whatever scope selected
        const single = [header];
        if (scope === "kpis") single.push(kpis);
        if (scope === "table") single.push(scheduleVar);
        if (scope === "taskPerformance") single.push(taskCard);
        if (scope === "charts") single.push(complianceRow, capaRow, riskRow);
        if (scope === "findings") single.push(findings);
        pageGroups.push(single.filter(isVisibleEl));
      }

      const allEls = [...new Set(pageGroups.flat())];
      if (!allEls.length) throw new Error("Nothing visible to export");

      await new Promise(requestAnimationFrame);

      // Render each element once
      const cache = new Map();
      ...
      pageGroups.forEach((group, idx) => {
        ...
        // Natural heights if rendered at full contentW
        const natural = group.map(el => {
          const c = cache.get(el);
          return { el, c, hAtFull: contentW * (c.h / c.w) };
        });
        const totalNatural = natural.reduce((s, n) => s + n.hAtFull, 0) + gap * (natural.length - 1);
        const scale = Math.min(1, contentH / totalNatural);
        const drawW = contentW * scale;
        const xOff = margin + (contentW - drawW) / 2;
        let y = margin;
        natural.forEach((n, i) => {
          const h = n.hAtFull * scale;
          pdf.addImage(n.c.dataUrl, "PNG", xOff, y, drawW, h, undefined, "FAST");
          y += h + (i < natural.length - 1 ? gap : 0);
        });
      });
