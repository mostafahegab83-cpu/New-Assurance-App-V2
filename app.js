...
(function () {
  ...
  function populateRecFilterOptions() {
    REC_FILTERS.forEach(([selId, field]) => {
      ...
      const vals = Array.from(new Set(
        records.map(r => (r[field] ?? "").toString().trim()).filter(v => v && v !== NA)
      )).sort((a,b) => a.localeCompare(b));
      sel.innerHTML = `<option value="">All</option>` +
        vals.map(v => `<option value="${escHtml(v)}"${v===current?" selected":""}>${escHtml(v)}</option>`).join("");
    });
  }
  function getFilteredRecords() {
    ...
    REC_FILTERS.forEach(([selId, field]) => {
      const sel = document.getElementById(selId);
      if (sel && sel.value) f[field] = sel.value;
    });
    return records.filter(r =>
      Object.entries(f).every(([k,v]) => (r[k] ?? "").toString().trim() === v)
    );
  }
