...
(function () {
  ...
  function populateRecFilterOptions() {
    REC_FILTERS.forEach(([selId, field]) => {
      ...
      const vals = Array.from(new Set(records.map(r => (r[field] ?? "")).map(v => String(v).trim()).filter(v => v && v !== "N/A"))).sort();
      sel.innerHTML = `<option value="">All</option>` + vals.map(v => `<option value="${escHtml(v)}"${v===current?" selected":""}>${escHtml(v)}</option>`).join("");
    });
  }
  function getFilteredRecords() {
    const filters = REC_FILTERS.map(([selId, field]) => [field, (document.getElementById(selId)?.value || "").trim()]).filter(([,v]) => v);
    if (!filters.length) return records;
    return records.filter(r => filters.every(([field, v]) => String(r[field] ?? "").trim() === v));
  }
  function wireRecFilters() {
    ...
    if (btn) btn.addEventListener("click", () => {
      REC_FILTERS.forEach(([selId]) => { const s = document.getElementById(selId); if (s) s.value = ""; });
      renderRecords();
    });
  }
  function renderRecords() {
    ...
    document.getElementById("recordsCount").textContent = `${filtered.length} of ${records.length} record(s)`;
    ...
    if (!filtered.length) {
      tbody.innerHTML = `<tr><td colspan="${colCount}" style="text-align:center;color:var(--muted);padding:24px">${records.length ? "No records match the selected filters." : "No records yet. Use the “Review form” tab to create one."}</td></tr>`;
      return;
    }
