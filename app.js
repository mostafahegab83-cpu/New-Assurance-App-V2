  ...
  async function exportPdf() {
    ...
    try {
      ...
      if (scope === "all" || scope === "kpis") {
        addBlock(capture.querySelector('[data-section="kpis"]'), { keepWithNext: true });
      }
      if (scope === "all" || scope === "findings") {
        addBlock(document.getElementById("findingsSection"));
      }
      ...
      for (let i = 0; i < rendered.length; i++) {
        ...
        const nextH = next ? next.naturalH + gap : 0;
        const neededH = drawH + (pageHasContent ? 0 : 0) + nextH;
        const remainingH = margin + contentH - cursorY;

        if (pageHasContent && neededH > remainingH && drawH <= contentH) {
          addNewPage();
        }
        ...
        if (pageHasContent && cursorY + drawH > margin + contentH) {
          addNewPage();
        }
