...
(function () {
  ...
  const FIELDS = [
    ...
    { id: "complianceReason", label: "NC Reasons",  kind: "raw"  },
  ...
  document.querySelectorAll(".tab").forEach(tab => {
    tab.addEventListener("click", () => {
      ...
      if (tab.dataset.tab === "form") setTimeout(() => lockUserNameField(), 0);
    });
  });
  ...
  function updateDataLists() {
    ...
    if (psel) {
      ...
      psel.innerHTML = `<option value="">All Processes</option>` +
        procs.map(v => `<option>${v.replace(/"/g,'&quot;')}</option>`).join("");
      psel.value = pcur;
    }
  }
  ...
  const REC_FILTERS = [
    ["fltAuditId", "auditId"],
    ["fltAuditDate", "auditDate"],
    ["fltUserName", "userName"],
    ["fltProcessName", "processName"],
    ["fltProcessPhase", "processPhase"],
    ["fltDayWeek", "dayWeek"],
    ["fltDepartment", "department"],
    ["fltSubDepartment", "subDepartment"]
  ];

  function recordFilterValue(r, field) {
    if (!r) return "";
    if (field === "auditId") return String(r.auditId || r.id || "").trim();
    if (field === "auditDate") {
      const raw = r.auditDate || r.reviewDate || r.createdAt || r.updatedAt || "";
      if (!raw) return "";
  ...
  function populateRecFilterOptions() {
    REC_FILTERS.forEach(([selId, field]) => {
      const sel = document.getElementById(selId);
      if (!sel) return;
      const current = sel.value;
      const vals = Array.from(new Set(records.map(r => recordFilterValue(r, field)).filter(v => v && v !== NA))).sort();
      sel.innerHTML = `<option value="">All</option>` + vals.map(v => `<option value="${escHtml(v)}"${v === current ? " selected" : ""}>${escHtml(v)}</option>`).join("");
    });
  }

  function getFilteredRecords() {
    const activeFilters = REC_FILTERS
      .map(([selId, field]) => [field, (document.getElementById(selId)?.value || "").trim()])
      .filter(([, value]) => value);
    if (!activeFilters.length) return records;
    return records.filter(r => activeFilters.every(([field, value]) => recordFilterValue(r, field) === value));
  }

  function wireRecFilters() {
    if (window.__recFiltersWired) return;
    window.__recFiltersWired = true;
    REC_FILTERS.forEach(([selId]) => {
      const sel = document.getElementById(selId);
      if (sel) sel.addEventListener("change", renderRecords);
    });
    const btn = document.getElementById("btnClearRecFilters");
  ...
  function renderRecords() {
    ...
    populateRecFilterOptions();
    wireRecFilters();
    const filteredRecords = getFilteredRecords();
    document.getElementById("recordsCount").textContent = `${filteredRecords.length} of ${records.length} record(s)`;
    if (!filteredRecords.length) {
      tbody.innerHTML = `<tr><td colspan="25" style="text-align:center;color:var(--muted);padding:24px">${records.length ? "No records match the selected filters." : "No records yet. Use the “Add / Edit” tab to create one."}</td></tr>`;
      return;
    }
  ...
  function getFiltered() {
    ...
    const proc = (document.getElementById("filterProcess")||{value:""}).value;
    ...
    return records.filter(r => {
      ...
      if (proc && r.processName !== proc) return false;
  ...
  ["filterSearch","filterDepartment","filterProcess","filterCompliance","filterRiskLevel","filterMitigation"]
    .forEach(id => { const el = document.getElementById(id); if (el) el.addEventListener("input", renderDashboard); });
  ...
  function renderDashboard() {
    ...
    drawReasonPie("chartNcReasons", counts(
      data.filter(r => r.compliance === "Non-Compliant" || r.compliance === "Partially Compliant"),
      "complianceReason", false
    ));
  ...
  function exportCSV() {
    const recFilterActive = typeof getFilteredRecords === "function" && REC_FILTERS.some(([selId]) => document.getElementById(selId)?.value);
    const dashboardFilterActive = ["filterSearch","filterDepartment","filterProcess","filterCompliance","filterRiskLevel","filterMitigation"]
      .some(id => document.getElementById(id).value);
    const data = recFilterActive ? getFilteredRecords() : (dashboardFilterActive ? getFiltered() : records);
