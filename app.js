...
(function () {
  ...
  /* ---------- Records table ---------- */
  const REC_FILTER_IDS = [
    ["fltAuditId","auditId"],["fltAuditDate","auditDate"],["fltUserName","userName"],
    ["fltProcessName","processName"],["fltProcessPhase","processPhase"],["fltDayWeek","dayWeek"],
    ["fltDepartment","department"],["fltSubDepartment","subDepartment"],
  ];
  function recFieldValue(r, field){
    let v = r[field];
    if ((v == null || v === "" || v === NA) && field === "auditId") v = r.id;
    if ((v == null || v === "" || v === NA) && field === "auditDate") v = r.createdAt || r.updatedAt;
    return v == null ? "" : String(v);
  }
  function populateRecFilterOptions(){
    REC_FILTER_IDS.forEach(([selId, field]) => {
      const sel = document.getElementById(selId);
      if (!sel) return;
      const current = sel.value;
      const vals = Array.from(new Set(records.map(r => recFieldValue(r, field)).filter(v => v && v !== NA))).sort();
      const firstOpt = sel.querySelector('option[value=""]');
      const placeholder = firstOpt ? firstOpt.outerHTML : `<option value="">All</option>`;
      sel.innerHTML = placeholder + vals.map(v => `<option value="${escHtml(v)}">${escHtml(v)}</option>`).join("");
      if (vals.includes(current)) sel.value = current;
    });
  }
  function getRecFilterValues(){
    const out = {};
    REC_FILTER_IDS.forEach(([selId, field]) => {
      const sel = document.getElementById(selId);
      if (sel && sel.value) out[field] = sel.value;
    });
    return out;
  }
  function applyRecFilters(list){
    const f = getRecFilterValues();
    const keys = Object.keys(f);
    if (!keys.length) return list;
    return list.filter(r => keys.every(k => recFieldValue(r, k) === f[k]));
  }
  function wireRecFilters(){
    REC_FILTER_IDS.forEach(([selId]) => {
      const sel = document.getElementById(selId);
      if (!sel || sel.dataset.wired) return;
      sel.dataset.wired = "1";
      sel.addEventListener("change", renderRecords);
    });
    const clr = document.getElementById("btnClearFilters");
    if (clr && !clr.dataset.wired) {
      clr.dataset.wired = "1";
      clr.addEventListener("click", () => {
        REC_FILTER_IDS.forEach(([selId]) => { const s = document.getElementById(selId); if (s) s.value = ""; });
        renderRecords();
      });
    }
  }
  ...
  function renderRecords() {
    ...
    wireRecFilters();
    const filtered = applyRecFilters(records);
    document.getElementById("recordsCount").textContent =
      filtered.length === records.length ? `${records.length} record(s)` : `${filtered.length} of ${records.length} record(s)`;
    ...
    if (!filtered.length) {
      const msg = records.length ? "No records match the selected filters." : "No records yet. Use the “Audit form” tab to create one.";
