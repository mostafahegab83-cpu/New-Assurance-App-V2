...
(function () {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      const pushChartBlock = el => {
        if (!isVisible(el) || chartBlocks.includes(el)) return;
        chartBlocks.push(el);
      };
      const chartIds = [
      ...
      if (scope === "all" || scope === "charts") {
        chartIds.forEach((id) => pushChartBlock(getChartCard(id)));
      ...
      if (renderedCharts.length) {
        ...
        for (let i = 0; i < renderedCharts.length; i += cols) {
          const row = renderedCharts.slice(i, i + cols);
