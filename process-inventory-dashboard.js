...
(function () {
  ...
  function render() {
    ...
    bar("pidChartShared", ["1 Company","2 Companies","3 Companies","4 Companies","5+ Companies"],
        Object.values(sharedBuckets), { color: "#7c3aed" });
    ...
    bar("pidChartStd", stdEntries.map(x=>x[0]), stdEntries.map(x=>x[1]), { color: "#14b8a6" });
    ...
    /* Top 10 shared */
    const topShared = [...records].sort((a,b)=>(b.companies||[]).length - (a.companies||[]).length).slice(0,10);
    $("pidTopShared").innerHTML = topShared.map(r => `
      <tr><td>${esc(r.documentTitle)}</td><td>${esc(r.documentCode)}</td>
    ...
    /* Oldest */
    const oldest = [...records].filter(r=>r.issueDate).sort((a,b)=>a.issueDate.localeCompare(b.issueDate)).slice(0,8);
    $("pidOldest").innerHTML = oldest.map(r => `
      <tr><td>${esc(r.documentTitle)}</td><td>${esc(r.issueDate)}</td><td>${esc(r.issueNo)}</td></tr>`).join("")
      || `<tr><td colspan="3" style="color:#9ca3af;text-align:center;padding:12px;">No data</td></tr>`;
    ...
    const missing = records.map(r => {
      const has = new Set((r.companies||[]).map(norm));
      const miss = allCompanies.filter(c => !has.has(norm(c)));
      return { r, miss };
    }).filter(x => x.miss.length && x.miss.length < allCompanies.length)
      .sort((a,b)=>a.miss.length - b.miss.length).slice(0,8);
    $("pidMissing").innerHTML = missing.map(({r,miss}) => `
      <tr><td>${esc(r.documentTitle)}</td><td style="color:#dc2626;">${esc(miss.slice(0,3).join(", "))}${miss.length>3?` +${miss.length-3}`:""}</td></tr>`).join("")
      || `<tr><td colspan="2" style="color:#9ca3af;text-align:center;padding:12px;">All processes have full coverage</td></tr>`;
