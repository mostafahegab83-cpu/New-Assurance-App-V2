...
function openFiles(id){
  ...
  `;
  document.getElementById("dcHistoryModal").querySelector("h3").textContent = "Attached Files";
...
function init(){
  ...
  document.querySelector("#dcTable")?.addEventListener("click", (e)=>{
    ...
    if (editId){ const d=DOCS.find(x=>x.id===editId); if(d) openModal(d); }
      ...
      document.getElementById("dcHistoryModal").querySelector("h3").textContent = "Version History";
      openHistory(hisId);
    }
