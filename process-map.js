...
(function(){
  ...
  // Normalize SLA display: if raw value already has letters (e.g. "2 days", "3 hrs"),
  // return as-is; otherwise append a single "d" suffix. Prevents "2 daysd".
  ...
  function computeLayout(){
    ...
    // Pre-scan: how many steps sit in each (dept|week) cell, and derive
    // how wide each column needs to be to fit them side-by-side.
    const cellTotal = {};
    state.steps.forEach(s => {
      const key = s.department+'|'+s.week;
      cellTotal[key] = (cellTotal[key]||0) + 1;
    });
    const colMax = {};
    state.timeline.forEach(t => { colMax[t.week] = 1; });
    Object.entries(cellTotal).forEach(([key, count]) => {
      const wk = key.split('|')[1];
      if (count > (colMax[wk]||1)) colMax[wk] = count;
    });
    const slotW = L.boxW + 18; // horizontal spacing between boxes in a cell
    const colWFor = wk => Math.max(L.colW, colMax[wk] * slotW + 20);

    // Week x map with dynamic widths, laid out left-to-right in timeline order.
    const weekX = {}; const weekW = {};
    let cursor = L.laneHeaderW + L.padL;
    state.timeline.forEach(t => {
      weekX[t.week] = cursor;
      weekW[t.week] = colWFor(t.week);
      cursor += weekW[t.week];
    });
    const totalW = cursor + L.padR;

    // Lane heights: expand if any cell in this lane needs multiple rows.
    // (We keep single-row per cell for now; heights stay uniform.)
    const laneY = {}; let y = L.headerH + L.weekHeaderH;
    state.departments.forEach(d => {
      const h = state.collapsed[d.name] ? collapsedH : L.laneH;
      laneY[d.name] = { y, h, collapsed: !!state.collapsed[d.name] };
      y += h;
    });
    const totalH = y + 20;

    // Per-cell horizontal placement
    const cellCount = {};
    const stepPos = {};
    state.steps.forEach(s => {
  ...
  function render(){
    ...
    state.timeline.forEach((t,i) => {
      const x = weekX[t.week]; const w = weekW[t.week] || L.colW;
