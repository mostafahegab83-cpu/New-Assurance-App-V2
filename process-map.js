/* Process Map Generator v3 — dynamic swimlane engine
 *
 * Primary input (new unified schema, single worksheet):
 ...
 *
 * Features:
 *  - Auto-detects swimlanes from the Swimlane column (order of first
 *    appearance) and auto-sizes the canvas.
 *  - Auto-detects sequence columns from the Sequence column.
 *  - Multi-process support: groups rows by Process ID, exposes a selector.
 *  - Validation: duplicate Step IDs, missing Parent/Next, single Start,
 *    at least one End, unique Sequence, valid Step Types.
 *  - Draws Start/End (capsule), Process/Approval (rounded rect),
 *    Decision (diamond), Document (document shape).
 *  - Arrows follow Next Step(s); Decision arrows carry Branch Label.
 *  - Annotation → floating dashed note next to the step.
 *  - Zoom/pan/drag, PNG/SVG/PDF/VDX export, Excel template download.
 *
 * Legacy fallback: 3-sheet workbooks (ProcessSteps/Departments/Timeline)
 * and legacy TSV paste (Seq/Activity/Role/Approver/SLA) still work.
 */
(function(){
  // ---------- Constants ----------
  ...
  // ---------- State ----------
  const state = {
    processes: {},      // pid -> { pid, title, no, ver, steps:[], departments:[], sequences:[] }
    processOrder: [],   // ordered list of pids
    currentPid: null,
    steps: [],          // active process steps
    departments: [],    // active departments {name,color,order}
    sequences: [],      // active sequence columns [{key,label,x}]
    positions: {},      // stepId -> {x,y} user drag override (per-process reset)
  ...
  function buildProcess(g){
    ...
    // Auto-derive Previous from Next relationships if not provided
    ...
    // If some steps have no seq (e.g. notes), give them the parent's seq
    steps.forEach(s => {
      if (s.seq == null){
  ...
  function validate(){
    ...
    // Sequence uniqueness (per flow step; notes may share)
    const seqSeen = {};
    state.steps.forEach(s => { if (s.type!=='note' && s.seq!=null) seqSeen[s.seq] = (seqSeen[s.seq]||0)+1; });
    // NOTE: multiple steps may share a sequence when they live in different swimlanes;
    // only warn if the same swimlane has duplicates
    const laneSeq = {};
    state.steps.forEach(s => {
  ...
  function loadFromWorkbookLegacy(wb){
    ...
    // Build simple linear nexts from parent
    const byId = {}; steps.forEach(s=>byId[s.id]=s);
    steps.forEach(s => s.parent.forEach(p => { if (byId[p]) byId[p].nexts.push(s.id); }));

    const p = { pid:'legacy', title:'', no:'', ver:'', steps,
      departments: [], sequences: [], byId };
    // Departments
    const seen = new Set();
  ...
  // ---------- Layout ----------
  const LAYOUT = {
    laneHeaderW: 160, headerH: 60, seqHeaderH: 34,
    laneH: 150, colW: 220, padL: 12, padR: 60,
    boxW: 180, boxH: 78
  };

  function computeLayout(){
    const L = LAYOUT;
    const laneY = {}; let y = L.headerH + L.seqHeaderH;
    const collapsedH = 22;
    state.departments.forEach(d => {
      const isCol = !!state.collapsed[d.name];
      laneY[d.name] = { y, h: isCol ? collapsedH : L.laneH, collapsed: isCol };
      y += isCol ? collapsedH : L.laneH;
    });

    const slotW = L.boxW + 22;
    // For each (dept, seq) count
    const cellCount = {};
    state.steps.forEach(s => {
    ...
    // seq column width = max over lanes of cell count
    ...
    // Position each shape (per-cell auto placement)
    const cellIndex = {};
    const stepPos = {};
    state.steps.forEach(s => {
      if (s.type==='note') return;
      const lane = laneY[s.department] || { y:L.headerH+L.seqHeaderH, h:L.laneH, collapsed:false };
    ...
    return { totalW, totalH, laneY, seqX, seqW, stepPos };
  }
  ...
  function render(){
    ...
    // sync title/no/ver from inputs (user edits win)
    ...
    const { totalW, totalH, laneY, seqX, seqW, stepPos } = layout;
    ...
    // Sequence header
    s += `<rect x="0" y="${L.headerH}" width="${L.laneHeaderW}" height="${L.seqHeaderH}" fill="#eef2f7" stroke="#1f4e79"/>`;
    s += `<text x="${L.laneHeaderW/2}" y="${L.headerH+L.seqHeaderH/2+4}" text-anchor="middle" font-size="11" font-weight="bold" fill="#1f3a8a">Swimlane \\ Sequence</text>`;
    state.sequences.forEach(sq => {
      const x = seqX[sq.key], w = seqW[sq.key];
      s += `<rect x="${x}" y="${L.headerH}" width="${w}" height="${L.seqHeaderH}" fill="#eef2f7" stroke="#1f4e79"/>`;
      s += `<text x="${x+w/2}" y="${L.headerH+L.seqHeaderH/2+4}" text-anchor="middle" font-size="12" font-weight="bold" fill="#1f3a8a">${esc(sq.label)}</text>`;
    });
    ...
    state.departments.forEach(d => {
      ...
      if (lane.collapsed){
        ...
        s += `<text x="10" y="${lane.y+lane.h/2+4}" font-size="11" font-weight="bold" fill="#1f3a8a" style="pointer-events:none">▸ ${esc(d.name)} (collapsed)</text>`;
        return;
      }
      ...
      state.sequences.forEach(sq => {
        const x = seqX[sq.key] + seqW[sq.key];
        s += `<line x1="${x}" y1="${lane.y}" x2="${x}" y2="${lane.y+lane.h}" stroke="#e3e8ef"/>`;
      });
    });

    // Connectors: use Next Step(s)
    ...
    state.steps.forEach(st => {
      ...
      const nexts = st.nexts.length ? st.nexts : []; // notes/annotations don't drive flow
      nexts.forEach((n, i) => {
    ...
    // Annotations (floating dashed connections)
    state.steps.forEach(st => {
      if (!st.annotation) return;
      const p = stepPos[st.id]; if (!p) return;
      const nx = p.x + L.boxW + 30;
      const ny = p.y - 50;
      const noteW = 150, noteH = 40;
      s += `<line x1="${p.x+L.boxW}" y1="${p.y+10}" x2="${nx}" y2="${ny+noteH/2}" stroke="#b45309" stroke-width="1" stroke-dasharray="4 3"/>`;
    ...
    state.steps.forEach(st => {
      ...
      const lines = wrap(label, isDec?16:26).slice(0, isDec?3:3);
      s += `<text x="${p.x+L.boxW/2}" y="${p.y+14}" text-anchor="middle" font-size="10" font-weight="bold" fill="#0f172a" style="pointer-events:none">${esc(st.id)}${isDec?' ?':''}</text>`;
      lines.forEach((ln,li) => {
  ...
  function downloadVsdx(){
    // Minimal VDX export (best effort — same as v41)
    alert('VDX export is available in Excel/PDF via the other buttons; for full Visio import, use the PNG/PDF or the .vsdx generator in your desktop tool.');
  }
