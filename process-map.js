...
(function(){
  ...
  function sheetHasStepHeaders(ws){
    const rows = XLSX.utils.sheet_to_json(ws, { header:1, defval:'', raw:false });
    return rows.slice(0, 5).some(row => {
      const header = row.map(normHeader).join('|');
      return /stepid/.test(header) && /(stepname|activity)/.test(header);
    });
  }

  function findProcessSheet(wb){
    const named = findSheet(wb, 'ProcessSteps', 'Process Steps', 'Steps');
    if (named) return named;
    for (const name of (wb.SheetNames || [])){
      const ws = wb.Sheets[name];
      if (ws && sheetHasStepHeaders(ws)) return ws;
    }
    return null;
  }
  ...
  function loadFromWorkbook(wb){
    const psSheet = findProcessSheet(wb);
  ...
  function normalizeStep(r){
    ...
    return {
      ...
      department: String(r.department ?? r.departmentswimlane ?? r.swimlane ?? r.role ?? 'General').trim(),
  ...
  function computeLayout(){
    ...
    // Size each week column by the busiest department/week cell so boxes never overlap.
    const slotW = L.boxW + 18;
    const colMax = {};
    state.timeline.forEach(t => { colMax[t.week] = 1; });
    const preCount = {};
    state.steps.forEach(s => {
      const wk = s.week || state.timeline[0]?.week || 'Week 1';
      const key = `${s.department}|${wk}`;
      preCount[key] = (preCount[key] || 0) + 1;
      colMax[wk] = Math.max(colMax[wk] || 1, preCount[key]);
    });
    ...
    state.timeline.forEach(t => {
      weekX[t.week] = x;
      weekW[t.week] = Math.max(L.colW, (colMax[t.week] || 1) * slotW + 40);
      x += weekW[t.week];
    });
    ...
    // Per-cell auto placement for shapes in same (dept, week), left-to-right.
    const cellCount = {};
    const stepPos = {};
    state.steps.forEach(s => {
  ...
  function render(){
    ...
    const { totalW, totalH, laneY, weekX, weekW, stepPos } = layout;
    ...
    state.timeline.forEach((t,i) => {
      ...
      const w = weekW[t.week] || L.colW;
      s += `<rect x="${x}" y="${L.headerH}" width="${w}" height="${L.weekHeaderH}" fill="#eef2f7" stroke="#1f4e79"/>`;
      s += `<text x="${x + w/2}" y="${L.headerH + L.weekHeaderH/2 + 4}" text-anchor="middle" font-size="12" font-weight="bold" fill="#1f3a8a">${esc(t.week)}</text>`;
    });
    ...
    state.departments.forEach(d => {
      ...
      state.timeline.forEach(t => {
        const x = weekX[t.week] + (weekW[t.week] || L.colW);
    ...
    const drawConn = (fromId, toId, kind, labelOverride) => {
      ...
      const label = labelOverride || (to.sla ? `SLA: ${formatSla(to.sla)}` : '');
      if (label){
        const mx = (a.x + b.x)/2, my = (a.y + b.y)/2 - 6;
        const labelW = Math.max(56, label.length * 6 + 12);
        out += `<rect x="${mx-labelW/2}" y="${my-11}" width="${labelW}" height="15" rx="3" fill="#fff" stroke="${color}" stroke-width="0.6"/>`;
    ...
    state.steps.forEach(st => {
      if (st.type==='decision'){
        ...
        s += drawConn(parseInt(st.prev), st.id, 'plain', st.sla ? `SLA: ${formatSla(st.sla)}` : '');
      }
    });
    ...
    state.steps.forEach(st => {
      ...
      if (!isDec){
        ...
        if (st.sla) s += `<text x="${p.x + L.boxW/2}" y="${p.y + L.boxH - 4}" text-anchor="middle" font-size="9" fill="#475569" style="pointer-events:none">SLA: ${esc(formatSla(st.sla))}</text>`;
      }
      // Document icon
      if (st.document){
...
<Shapes>${shapes}</Shapes></Page></Pages></VisioDocument>`;
  ...
  async function handleExcel(file, token){
    ...
    const svg = document.getElementById('pmMap');
    if (svg){ svg.innerHTML = ''; svg.removeAttribute('viewBox'); }
    try {
      const buffer = await file.arrayBuffer();
      if (token !== lastExcelUploadToken) return;
      const data = new Uint8Array(buffer);
      const wb = XLSX.read(data, { type:'array', cellDates:true });
      // Previous data was cleared before reading so failed uploads cannot leave stale rows behind.
      const ok = loadFromWorkbook(wb);
      if (!ok){
        // Legacy fallback: try each sheet as a TSV table until one parses
        let parsed = false;
        let lastTsv = '';
        for (const name of (wb.SheetNames || [])){
          const ws = wb.Sheets[name];
          ...
          lastTsv = tsv;
          if (parseLegacyText(tsv)){ parsed = true; break; }
        }
        if (ta){ ta.value = lastTsv; ta.dataset.dirty = ''; }
        if (!parsed){
  ...
  function generate(){
    // Prefer the current upload unless the user has edited the paste box.
    const ta = document.getElementById('pmData');
  ...
  function init(){
    ...
    const ta = document.getElementById('pmData');
    if (ta){
      // Do not let the built-in browser form restore or old template sample data repopulate after refresh.
      ta.value = '';
      ta.defaultValue = '';
      ta.dataset.dirty = '';
    ...
    if (tab){
      tab.addEventListener('click', () => {
        const svg = document.getElementById('pmMap');
        const ta = document.getElementById('pmData');
        if (svg && !svg.innerHTML.trim() && (state.steps.length || (ta && ta.value.trim()))) generate();
      });
    }
  }
