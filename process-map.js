...
(function(){
  ...
  function downloadVsdx(){
    if (!window.JSZip){ alert('JSZip not loaded — cannot build Visio file.'); return; }
    if (!state.steps || !state.steps.length){ alert('No data to export.'); return; }

    const lay = computeLayout();
    const PX = 1/96;
    const pageW = Math.max(8.5, (lay.totalW||1200) * PX);
    const pageH = Math.max(11,  (lay.totalH||900)  * PX);

    const esc = t => String(t==null?'':t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
    const wrap = (t,n) => { const w=String(t||'').split(/\s+/); const out=[]; let l=''; w.forEach(x=>{ if((l+' '+x).trim().length>n){out.push(l);l=x;} else l=(l?l+' ':'')+x; }); if(l)out.push(l); return out.join('\n'); };

    // Assign IDs
    const idMap = {}; let nextId = 2; // 1 reserved for lane container if we add
    state.steps.forEach(st => { if (st.type==='note') return; idMap[st.id]=nextId++; });

    // Geometry helpers per type
    function geomRect(){
    ...
    function geomEllipse(){
      return `<Section N="Geometry" IX="0"><Cell N="NoFill" V="0"/><Cell N="NoLine" V="0"/>`+
        `<Row T="Ellipse" IX="1"><Cell N="X" F="Width*0.5"/><Cell N="Y" F="Height*0.5"/>`+
        `<Cell N="A" F="Width"/><Cell N="B" F="Height*0.5"/><Cell N="C" F="Width*0.5"/><Cell N="D" F="Height"/></Row></Section>`;
    }
    function geomDiamond(){
      return `<Section N="Geometry" IX="0"><Cell N="NoFill" V="0"/><Cell N="NoLine" V="0"/>`+
        ...
        `<Row T="RelLineTo" IX="5"><Cell N="X" V="0.5"/><Cell N="Y" V="0"/></Row></Section>`;
    }

    const fillFor = t => t==='decision' ? 'RGB(255,236,179)' : t==='start'||t==='end' ? 'RGB(200,230,201)' : t==='approval' ? 'RGB(255,224,178)' : t==='document' ? 'RGB(225,245,254)' : 'RGB(222,234,246)';

    // Build shapes
    let shapes = '';
    state.steps.forEach(st => {
      if (st.type==='note') return;
      const pos = lay.stepPos[st.id]; if (!pos) return;
      const bw = 160, bh = 70; // default box in px
      const cxPx = pos.x + bw/2;
      const cyPx = pos.y + bh/2;
      const pinX = cxPx * PX;
      const pinY = pageH - cyPx * PX;
      const w = bw * PX;
      const h = bh * PX;
      const id = idMap[st.id];
      const geom = st.type==='decision' ? geomDiamond() : (st.type==='start'||st.type==='end') ? geomEllipse() : geomRect();
      const label = esc(wrap(st.name||st.id, 22));
      shapes += `<Shape ID="${id}" NameU="${esc(st.id)}" Type="Shape" LineStyle="0" FillStyle="0" TextStyle="0">`+
        `<Cell N="PinX" V="${pinX.toFixed(3)}"/><Cell N="PinY" V="${pinY.toFixed(3)}"/>`+
        `<Cell N="Width" V="${w.toFixed(3)}"/><Cell N="Height" V="${h.toFixed(3)}"/>`+
        `<Cell N="LocPinX" F="Width*0.5"/><Cell N="LocPinY" F="Height*0.5"/>`+
        `<Cell N="FillForegnd" V="${fillFor(st.type)}"/>`+
        `<Cell N="LineColor" V="RGB(30,58,138)"/><Cell N="LineWeight" V="0.015"/>`+
        `${geom}<Text>${label}</Text></Shape>`;
    });
    ...
    state.steps.forEach(st => {
      if (st.type==='note') return;
      const fromId = idMap[st.id]; if (!fromId) return;
      const fp = lay.stepPos[st.id]; if (!fp) return;
      (st.nexts||[]).forEach((nid, ni) => {
        const toId = idMap[nid]; if (!toId) return;
        const tp = lay.stepPos[nid]; if (!tp) return;
        const cid = nextId++;
        const bx = (fp.x+80) * PX, by = pageH - (fp.y+35) * PX;
        const ex = (tp.x+80) * PX, ey = pageH - (tp.y+35) * PX;
        const lbl = esc((st.branchLabels && st.branchLabels[ni]) || '');
        const isYes = /yes|approv/i.test(lbl), isNo = /no|reject/i.test(lbl);
        const lc = isYes ? 'RGB(46,125,50)' : isNo ? 'RGB(198,40,40)' : 'RGB(55,71,79)';
        connects += `<Shape ID="${cid}" NameU="Dynamic connector" Type="Shape">`+
          `<Cell N="PinX" V="0"/><Cell N="PinY" V="0"/><Cell N="Width" V="1"/><Cell N="Height" V="0"/>`+
          `<Cell N="LocPinX" V="0"/><Cell N="LocPinY" V="0"/>`+
          `<Cell N="BeginX" V="${bx.toFixed(3)}"/><Cell N="BeginY" V="${by.toFixed(3)}"/>`+
          `<Cell N="EndX" V="${ex.toFixed(3)}"/><Cell N="EndY" V="${ey.toFixed(3)}"/>`+
          `<Cell N="ConLineRouteExt" V="2"/><Cell N="ShapeRouteStyle" V="16"/>`+
          `<Cell N="LineColor" V="${lc}"/><Cell N="LineWeight" V="0.02"/>`+
          `<Cell N="EndArrow" V="4"/>`+
          `<Section N="Geometry" IX="0"><Cell N="NoFill" V="1"/><Cell N="NoLine" V="0"/>`+
          `<Row T="MoveTo" IX="1"><Cell N="X" F="BeginX"/><Cell N="Y" F="BeginY"/></Row>`+
          `<Row T="LineTo" IX="2"><Cell N="X" F="EndX"/><Cell N="Y" F="EndY"/></Row></Section>`+
          (lbl ? `<Text>${lbl}</Text>` : '')+
          `</Shape>`;
      });
    });
    ...
    state.steps.forEach(st => {
      ...
      const id = nextId++;
      const w = 1.5, h = 0.6;
      const pinX = (pos.x + 200) * PX + w/2;
      const pinY = pageH - (pos.y + 35) * PX;
      shapes += `<Shape ID="${id}" NameU="Note" Type="Shape">`+
        `<Cell N="PinX" V="${pinX.toFixed(3)}"/><Cell N="PinY" V="${pinY.toFixed(3)}"/>`+
        `<Cell N="Width" V="${w}"/><Cell N="Height" V="${h}"/>`+
        `<Cell N="LocPinX" F="Width*0.5"/><Cell N="LocPinY" F="Height*0.5"/>`+
        `<Cell N="FillForegnd" V="RGB(255,253,231)"/><Cell N="LineColor" V="RGB(158,158,158)"/>`+
        `<Cell N="LinePattern" V="2"/>`+
        geomRect()+`<Text>${esc(wrap(st.annotation,26))}</Text></Shape>`;
    });
    ...
    const pageXML = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>`+
      ...
      `<Shapes>${shapes}${connects}</Shapes></PageContents>`;
    ...
    const pagesXML = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>`+
      ...
      `<Page ID="0" NameU="Page-1" Name="Page-1" ViewScale="-1" ViewCenterX="${(pageW/2).toFixed(3)}" ViewCenterY="${(pageH/2).toFixed(3)}">`+
      `<PageSheet LineStyle="0" FillStyle="0" TextStyle="0">`+
      `<Cell N="PageWidth" V="${pageW.toFixed(3)}"/><Cell N="PageHeight" V="${pageH.toFixed(3)}"/>`+
      `<Cell N="ShdwOffsetX" V="0.125"/><Cell N="ShdwOffsetY" V="-0.125"/>`+
      `<Cell N="PageScale" V="1" U="IN_F"/><Cell N="DrawingScale" V="1" U="IN_F"/>`+
      `<Cell N="DrawingSizeType" V="3"/><Cell N="DrawingScaleType" V="0"/><Cell N="InhibitSnap" V="0"/>`+
    ...
    const documentXML = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>`+
      ...
      `<GlyphSettingEntry/></DocumentSettings>`+
      `<Colors><ColorEntry IX="0" RGB="#000000"/><ColorEntry IX="1" RGB="#FFFFFF"/></Colors>`+
      `<FaceNames><FaceName NameU="Calibri" UnicodeRanges="0" CharSets="0" Panos="2 15 5 2 2 2 4 3 2 4" Flags="325"/></FaceNames>`+
      `<StyleSheets>`+
      `<StyleSheet ID="0" NameU="No Style" Name="No Style"><Cell N="EnableLineProps" V="1"/><Cell N="EnableFillProps" V="1"/><Cell N="EnableTextProps" V="1"/></StyleSheet>`+
    ...
    const appXML = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>`+
      `<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>Lovable Process Map</Application></Properties>`;
    const coreXML = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>`+
      `<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/">`+
      `<dc:title>${esc(state.title||'Process Map')}</dc:title><dc:creator>Lovable</dc:creator></cp:coreProperties>`;
    ...
    zip.generateAsync({ type:'blob', mimeType:'application/vnd.ms-visio.drawing' }).then(blob => {
      ...
      a.href = url; a.download = (state.title||'process-map').replace(/[^\w\-]+/g,'_') + '.vsdx';
