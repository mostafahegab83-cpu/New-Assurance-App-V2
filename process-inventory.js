...
(function () {
  ...
  function renderTable() {
    ...
    if (!pageRows.length) {
      tbody.innerHTML = `<tr><td colspan="10" style="text-align:center;color:#6b7280;padding:24px">No processes found.</td></tr>`;
    } else {
  ...
  function startSync() {
    ...
    unsubPi = onSnapshot(colRef, snap => {
      ...
      toastMsg("Process Inventory sync error");
    });
  }
  function stopSync() { if (unsubPi) { unsubPi(); unsubPi = null; } }
  ...
  async function saveModal(e) {
    ...
    try {
      ...
      console.error(err);
      toastMsg("Save failed: " + err.message);
    }
  }
  ...
  async function deleteRecord(id) {
    ...
    try {
      ...
      console.error(err); toastMsg("Delete failed: " + err.message);
    }
  }
