...
(function () {
  ...
  function renderTable() {
    ...
    if (piSyncState === "error") {
      tbody.innerHTML = `<tr><td colspan="12" style="text-align:center;color:#b91c1c;padding:24px;white-space:normal;">${esc(piSyncMessage)}</td></tr>`;
      const pager = $("piPager");
    ...
    if (piSyncState === "loading") {
      tbody.innerHTML = `<tr><td colspan="12" style="text-align:center;color:#6b7280;padding:24px;">${esc(piSyncMessage)}</td></tr>`;
      const pager = $("piPager");
      if (pager) pager.innerHTML = "";
      updateBulkUI(0);
      return;
    ...
    if (!pageRows.length) {
      tbody.innerHTML = `<tr><td colspan="12" style="text-align:center;color:#6b7280;padding:24px">No processes found.</td></tr>`;
    } else {
